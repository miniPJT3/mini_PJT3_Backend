...
public class StatService {
    ...
    public StatResponse getSellerStat(Long sellerId) {
        ...
        SellerSalesStat stat = sellerSalesStatRepository.findBySellerIdAndStatDate(sellerId, today)
        ...
        SellerSalesStat stat = sellerSalesStatRepository.findBySellerIdAndStatDate(sellerId, today)
                .orElseGet(() -> {
                    SellerSalesStat newStat = SellerSalesStat.builder()
                            ...
                            .statDate(today)
                            .build();
                    return sellerSalesStatRepository.save(newStat);
                });
    ...
    public StatResponse initializeStat(Long sellerId) {
        String today = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        if (!sellerSalesStatRepository.existsBySellerIdAndStatDate(sellerId, today)) {
            SellerSalesStat newStat = SellerSalesStat.builder()
                    .sellerId(sellerId)
                    .statDate(today)
                    .build();
            sellerSalesStatRepository.save(newStat);
        }
